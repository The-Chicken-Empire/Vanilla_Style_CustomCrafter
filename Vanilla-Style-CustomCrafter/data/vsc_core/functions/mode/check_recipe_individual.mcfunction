...
execute if score VSC.page_transition VSC.main matches 0 unless data block ~ ~ ~ Items[{tag:{vsc_key:"recipe"}}] run execute store result storage vsc_core:main tmp.frame int 1 run scoreboard players get @s VSC.frame
execute if score VSC.page_transition VSC.main matches 0 unless data block ~ ~ ~ Items[{tag:{vsc_key:"recipe"}}] run function vsc_core:mode/page_transition/recipe with storage vsc_core:main tmp
